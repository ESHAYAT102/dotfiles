...
CustomMouseArea {
    ...
    function cycleRightPanels(): void {
        if (screenState.sidebar) {
            screenState.sidebar = false;
            screenState.session = false;
        } else if (screenState.session) {
            screenState.session = false;
    ...
    MouseArea {
        ...
        width: Math.max(6, Config.border.thickness)
        z: 10000
        hoverEnabled: true
        acceptedButtons: root.fullscreen ? Qt.NoButton : Qt.LeftButton

        onEntered: {
            if (!root.screenState.session && !root.screenState.sidebar) {
                root.screenState.osd = true;
                root.panels.osd.hovered = true;
            }
        }
        onClicked: root.cycleRightPanels()
    }

...
ColumnLayout {
    ...
    function toggleStatusPopout(name: string): void {
        if (Hypr.monitorFor(root.screen) !== Hypr.focusedMonitor)
            return;
        ...
        for (let i = 0; i < repeater.count; i++) {
            ...
            const items = (entry.item as StatusIcons).items;
            const icon = items.children.find(item => item.name === name && item.active);
            if (!icon)
                return;
            ...
            popouts.currentCenter = icon.mapToItem(root, 0, icon.implicitHeight / 2).y;

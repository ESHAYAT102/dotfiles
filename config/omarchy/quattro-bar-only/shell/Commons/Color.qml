...
QtObject {
  ...
  property string windowBorderValue: ""
  ...
  function mergeShell() {
    ...
    if (windowBorderValue.length > 0) {
      merged["hyprland.active-border"] = windowBorderValue
      accent = windowBorderValue
    }
    shellValues = merged
  ...
  function loadWindowBorder(raw) {
    try {
      var custom = String(JSON.parse(raw).custom || "").split(/\s+/)[0]
      if (/^[0-9A-Fa-f]{8}$/.test(custom)) {
        windowBorderValue = "#" + custom.slice(2)
        mergeShell()
      }
    } catch (error) {
    }
  ...
  property Process windowBorderProbe: Process {
    id: windowBorderProbe
    command: ["hyprctl", "getoption", "general:col.active_border", "-j"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.loadWindowBorder(text)
    }
  }
  property Timer windowBorderTimer: Timer {
    interval: 2000
    running: true
    repeat: true
    triggeredOnStart: true
    onTriggered: if (!windowBorderProbe.running) windowBorderProbe.running = true
  }

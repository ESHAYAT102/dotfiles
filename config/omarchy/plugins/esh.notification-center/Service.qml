// Adapted from njpatel/omapager; see THIRD_PARTY.md for provenance and changes.
...
Item {
  ...
  property alias centerModel: centerRows
  property bool centerOpen: false
  property var forgottenKeys: ({})
  property bool historyCleared: false
  ListModel { id: centerRows }

  function centerIndexFor(key) {
    for (var i = 0; i < centerRows.count; i++)
      if (centerRows.get(i).key === key) return i
    return -1
  }

  function remember(row) {
    if (!row || !row.key || forgottenKeys[row.key]) return
    if (Number(row.ts) < Date.now() / 1000 - 7 * 86400) return
    var at = centerIndexFor(row.key)
    if (at >= 0) {
      centerRows.set(at, Store.normalise(row))
      return
    }
    at = 0
  ...
  function dismissCenter(key) {
    forgottenKeys[key] = true
    closeToast(key, "dismissed")
    var at = centerIndexFor(key)
    if (at >= 0) centerRows.remove(at)
    held = held.filter(function(row) { return row.key !== key })
    if (rowIndexFor(key) < 0) release(key)
    Store.write(storeProc, storeBin, "forget", null, [key])
  }
  ...
  function clearCenter() {
    ...
    clearAll("dismissed")
    held = []
    Store.write(storeProc, storeBin, "clear", null)
    heldRows = []
    heldRevision += 1
  }
  ...
  Process {
    id: centerRestoreProc
    command: [service.storeBin, "history", "200"]
    stdout: StdioCollector {
      onStreamFinished: {
        if (service.historyCleared) return
        var rows = Store.parseList(text)
        for (var i = 0; i < rows.length; i++) {
          if (service.centerIndexFor(rows[i].key) >= 0) continue
          rows[i].replyPath = ""
          service.remember(rows[i])
        }
      }
    }
  }
  ...
  // Only defer arrivals while typing a reply, so its field stays put.
  // Hovering pauses expiry but must not hide incoming notifications.
  property var held: []
  function holding() { return replyingKey !== "" }
  ...
  // Bound how long incoming notifications can wait behind a reply.
  Timer {
    id: heldTooLong
    interval: 30000
  ...
  function handleNotification(notification) {
    ...
    notification.closed.connect(function() {
      if (service.refs[key] === notification) {
        ...
        var at = service.centerIndexFor(key)
        if (at >= 0) centerRows.setProperty(at, "replyPath", "")
      }
    })
    ...
    // new card waits, and only while a reply is being typed.
  ...
  function showRow(row) {
    ...
    Qt.callLater(function() {
      if (service.forgottenKeys[key]) return
  ...
  function activate(key) {
    ...
    if (!handled) {
      ...
      var row = at >= 0 ? toasts.get(at) : centerAt >= 0 ? centerRows.get(centerAt) : null
      if (row) {
    ...
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        ...
        if (path && at >= 0) {
          ...
          var centerAt = service.centerIndexFor(job.key)
          if (centerAt >= 0) centerRows.setProperty(centerAt, "replyPath", path)
  ...
  Component.onCompleted: {
    centerRestoreProc.running = true
      ...
      Item {
        ...
        Repeater {
          ...
          Toast {
            ...
            paused: service.centerOpen || service.expanded || service.replyingKey !== ""

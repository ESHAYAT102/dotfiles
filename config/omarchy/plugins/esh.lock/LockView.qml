...
Item {
  ...
  readonly property int fieldFontSize: Math.round(Style.font.heading * 0.8)
  readonly property int passwordDotFontSize: Math.round(Style.font.heading * 0.95)
  readonly property int passwordDotLetterSpacing: Math.round(Style.font.heading * 0.13)
  ...
  Rectangle {
    ...
    // Clock -- mirrors the hyprlock.conf time label (font_size 150, top center).
    Text {
      id: clockLabel
    ...
    Timer {
      ...
      repeat: true
      running: true
      onTriggered: clockLabel.text = Qt.formatTime(new Date(), "hh:mm:ss AP")
    }

    // Date -- mirrors the hyprlock.conf date label (font_size 32, below clock).
    Text {
      id: dateLabel

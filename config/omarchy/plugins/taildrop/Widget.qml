...
Panel {
  ...
  function clampTargetIndex() {
    ...
    if (targets.length === 0 && focusSection === "targets") focusSection = sending ? "send" : "files"
  }
  ...
  function moveCursor(dx, dy) {
    ...
    if (dy > 0) {
      if (focusSection === "files") focusSection = targets.length > 0 ? "targets" : (sending ? "send" : "files")
      else if (focusSection === "targets" && selectedTargetIndex < targets.length - 1) selectedTargetIndex++
      else if (focusSection === "targets" && sending) focusSection = "send"
    } else {
  ...
  function activateCursor() {
    ...
    else if (focusSection === "send" && sending) taildrop.cancel()
  }
    ...
    PanelKeyCatcher {
      ...
      Column {
        ...
        ActionRow {
          ...
          width: parent.width
          section: "send"
          label: qsTr("Cancel transfer")
          detail: root.taildrop && root.taildrop.canceling ? qsTr("Canceling...") : qsTr("Stop the active transfer")
          selected: root.cursorActive && root.focusSection === "send"
          current: root.sending
          enabled: root.taildrop && root.taildrop.installed && !root.receiving && !root.taildrop.canceling
          onActivated: root.taildrop.cancel()
        }
      }
    }
  ...
  component TargetRow: CursorSurface {
    ...
    Row {
      ...
      Text {
        id: deviceIcon
        text: root.targetIcon(targetRow.target)
      ...
      Column {
        width: parent.width - deviceIcon.width - row.spacing
        spacing: Style.space(1)
